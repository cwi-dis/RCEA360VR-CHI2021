...
def annotation_fusion(cluster_videos):
    ...
    for i in range(8):  # 8 videos
        V_video = []
        A_video = []
        for j in range(60):  # 60s per video
            ...
            for k in range(n):
                ...
                if index_frame.shape[0] != 0:
                    ...
                    f_v = frame_data[:, 3].reshape([-1, 1])
                    f_a = frame_data[:, 4].reshape([-1, 1])
                    F_v.append(bayes_fusion(f_v)[0])
                    F_a.append(bayes_fusion(f_a)[0])
                    N_v.append(f_v.shape[0])
                    N_a.append(f_a.shape[0])
                else:
                    F_v.append(np.nan)
                    F_a.append(np.nan)
                    N_v.append(0)
        ...
        V.append(np.array(V_video))
        A.append(np.array(A_video))
    return V, A
...
def bounding_box(seg_data):
    ...
    pitch = seg_data[:, 8].reshape([-1, 1])
    yaw = seg_data[:, 9].reshape([-1, 1])
