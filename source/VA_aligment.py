...
def a_shift(annotation, p):
    ...
    a_s = np.concatenate((a, np.ones([p, 1]) * 5), axis=0)
    return a_s
        ...
        for j in range(32):  # for 32 users
            try:
                ...
                for k in range(3):  # for 3 features
                    ...
                    if np.isnan(corr[i, j, k, 0]):
                        print("v", i, j, k)
                    if np.isnan(corr[i, j, k, 1]):
                        print("a", i, j, k)
            except:
        ...
        for j in range(2):
            ...
            for k in range(a_min, a_max, 3):
                ...
                dwt_a = dtw_distance(a_shift(data_p["Arousal"], k),
                                     np.array(data_v[data_v.keys()[f_v_index[i] + 2]]))
                Dwt[i, j, int(k / 3) - 2, 0] = dwt_v
                Dwt[i, j, int(k / 3) - 2, 1] = dwt_a
                print(dwt_v, dwt_a)
            print(i, j)
